...
public class ProfileLogic {
	...
	public int createProfile(String aPortalId, String anApplicationId, String aNickName, String aFirstName, String aLastName,
		...
		try {
			...
			if (aPortalId == null || aPortalId.length() == 0 || !Java.isInt(aPortalId) || oase.get(Core.PORTAL, aPortalId) == null)
				throw new UtopiaException("No portalid or portal could not be found.");
			if (anApplicationId == null || anApplicationId.length() == 0 || !Java.isInt(anApplicationId) || oase.get(Core.APPLICATION, anApplicationId) == null)
				throw new UtopiaException("No applicationid or application could not be found.");
			...
			if (aConfirmationUrl == null || aConfirmationUrl.length() == 0)
				throw new UtopiaException("No confirmation url found.");

			// check if email address already exists
			Record[] people = oase.getFinder().queryTable(Person.TABLE_NAME, Person.EMAIL_FIELD + "='" + anEmail + "'", null, null);
			if (people != null && people.length > 0)
				throw new UtopiaException("This email address is already registered.", ErrorCode.__6207_Value_already_in_use);
	...
	public void updateProfile(String aPersonId, String aNickName, String aFirstName, String aLastName,
		...
		try {
			// first check required params
			if (aPersonId == null || aPersonId.length() == 0 || !Java.isInt(aPersonId))
				throw new UtopiaException("No personId found.");
			...
			if (person == null)
				throw new UtopiaException("No person found with id " + aPersonId, ErrorCode.__6004_Invalid_attribute_value);
			...
			if ((anEmail != null && anEmail.length() > 0) || (aPassword != null && aPassword.length() > 0)) {
				// check if email address already exists
				Record[] people = oase.getFinder().queryTable(Person.TABLE_NAME, Person.EMAIL_FIELD + "='" + anEmail + "'", null, null);
				if (people != null && people.length > 0 && Integer.parseInt(aPersonId) != people[0].getId())
					throw new UtopiaException("This email address is already registered.", ErrorCode.__6207_Value_already_in_use);
			...
			if (aPhotoId != null && aPhotoId.length() > 0 && Java.isInt(aPhotoId)) {
				...
				if (photo != null && photo.getTableName().equals(Medium.TABLE_NAME)) {
					...
					for (int i = 0; i < media.length; i++) {
						if (media[i] != null && photo.getId() != Integer.parseInt(aPhotoId)) {
	...
	public JXElement getProfile(String aPersonId) throws UtopiaException {
		try {
			if (aPersonId == null || aPersonId.length() == 0 || !Java.isInt(aPersonId))
				throw new UtopiaException("No valid personid found", ErrorCode.__6002_Required_attribute_missing);
