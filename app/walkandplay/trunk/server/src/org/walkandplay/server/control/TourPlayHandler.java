    ...
    public JXElement getTour(UtopiaRequest anUtopiaRequest) throws UtopiaException {
        try {
            ...
            if (id == null || id.length() == 0 || !Java.isInt(id)) {
                throw new UtopiaException("No tourid found");
            }
            ...
            if (inviteeElms != null) {
                for (int i = 0; i < inviteeElms.length; i++) {
                    ...
                    inviteeElm.setAttr(Constants.INVITATION_FIELD, Constants.INVITATION_PENDING);
                ...
                for (int i = 0; i < mediumElms.size(); i++) {
                    ...
                    if (mediumId != null && mediumId.length() > 0 && Java.isInt(mediumId)) {
                        if (type != null && type.length() > 0) {
                            if (type.equals(Constants.RELTAG_INTRO)) {
                                relateLogic.relate(tourId, Integer.parseInt(mediumId), Constants.RELTAG_INTRO);
                            } else if (type.equals(Constants.RELTAG_OUTRO)) {
                                relateLogic.relate(tourId, Integer.parseInt(mediumId), Constants.RELTAG_OUTRO);
                            }
                        } else {
