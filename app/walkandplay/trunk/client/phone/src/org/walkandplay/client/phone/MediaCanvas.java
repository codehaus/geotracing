...
public class MediaCanvas extends DefaultCanvas {
    ...
    private final static int POI_STAT = 2;
    private final static int POI_INPUT_STAT = 3;
            ...
            switch(screenStat){
                ...
                case TAG_STAT:
                    if(tagCloudSelected){
                        ...
                        if(mediumType == TEXT){
                            // create the poi
                            WP.traceCanvas.getTracer().getNet().addPOI(poiType, titleText, inputText);
                            //clear fields
                            inputText = "";
                            titleText = "";
                            tagText = "";
    ...
    public boolean uploadPhoto(){
        JXElement rsp = Net.getInstance().uploadMedium(titleText, "image", mimePhoto, time, dataPhoto, false);
    ...
    public boolean uploadAudio(){
        JXElement rsp = Net.getInstance().uploadMedium(titleText, "audio", mimeAudio, startTime, dataAudio, false);
        if(rsp == null || rsp.getTag().indexOf("nrsp")!=-1){
            return false;
        }
        return true;
