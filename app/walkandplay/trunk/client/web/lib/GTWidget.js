 ...
 *
 * $Id: GTWidget.js,v 1.10 2006-07-20 12:09:32 just Exp $
 */
...
/** Create new Medium object. */
	createMedium: function(id, name, desc, type, mime, time, lon, lat) {
		return new Medium(id, name, desc, type, mime, time, lon, lat);
	},
...
// Create new Tracer object and put in tracers array
	createTracer: function(name, lon, lat) {
		...
		if (tracer) {
			if (lon && lat) {
				tracer.setLocation(new GLatLng(lat, lon));
			}
			tracer.show();
			return tracer;
		}
		...
		tracer = new Tracer(name, GTW.TRACK_COLOR, GTW.TRACER_ICON_URL, point);
